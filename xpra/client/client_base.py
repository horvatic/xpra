...
class XpraClientBase(ServerInfoMixin, FilePrintMixin):
    ...
    def setup_connection(self, conn):
        netlog.warn("setup_connection(%s) timeout=%s, socktype=%s", conn, conn.timeout, conn.socktype)
    ...
    def verify_connected(self):
        if self.server_capabilities is None:
            ...
            self.warn_and_quit(1, "connection timed out")
    ...
    def make_hello_base(self):
        ...
        if self.encryption:
            ...
            key = self.get_encryption_key()
            if key is None:
                self.warn_and_quit(1, "encryption key is missing")
                return None
    ...
    def _process_connection_lost(self, _packet):
        ...
        if self.exit_code is None:
            self.warn_and_quit(1, "Connection lost")
    ...
    def validate_challenge_packet(self, packet):
        ...
        if salt_digest in ("xor", "des"):
            if not LEGACY_SALT_DIGEST:
                self.auth_error(1, "server uses legacy salt digest '%s'" % salt_digest)
                return False
    ...
    def send_challenge_reply(self, packet, password):
        ...
        if self.encryption:
            ...
            key = self.get_encryption_key()
            if key is None:
                self.auth_error(1, "the server does not use any encryption", "client requires encryption")
                return
    ...
    def _process_hello(self, packet):
        ...
        if not self.password_sent and self.has_password():
            self.warn_and_quit(1, "the server did not request our password")
            return
        try:
            ...
            if not self.server_connection_established():
                self.warn_and_quit(1, "failed to establish connection")
        except Exception as e:
            netlog.info("error in hello packet", exc_info=True)
            self.warn_and_quit(1, "error processing hello packet from server: %s" % e)
