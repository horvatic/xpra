...
from xpra.os_util import bytestostr, strtobytes, get_hex_uuid, PYTHON3, POSIX, OSX
...
class CommandConnectClient(GObjectXpraClient):
    ...
    def connect_with_timeout(self, conn):
        ...
        def send_hello(*args):
            log.error("send_hello")
            self.send_hello()
        glib.idle_add(send_hello)
...
class HelloRequestClient(SendCommandConnectClient):
    ...
    def timeout(self, *_args):
        self.warn_and_quit(1, "timeout: server did not disconnect us")
...
class ScreenshotXpraClient(CommandConnectClient):
    ...
    def timeout(self, *_args):
        self.warn_and_quit(1, "timeout: did not receive the screenshot")
    ...
    def _process_screenshot(self, packet):
        ...
        with open(self.screenshot_filename, 'wb') as f:
            f.write(img_data)
        self.warn_and_quit(0, "screenshot %sx%s saved to: %s" % (w, h, self.screenshot_filename))
...
class InfoXpraClient(CommandConnectClient):
    ...
    def timeout(self, *_args):
        self.warn_and_quit(1, "timeout: did not receive the info")
...
class ConnectTestXpraClient(CommandConnectClient):
    ...
    def timeout(self, *_args):
        self.warn_and_quit(1, "timeout: no server response")
...
class VersionXpraClient(HelloRequestClient):
    ...
    def do_command(self):
        ...
        if not v:
            self.warn_and_quit(1, "server did not provide the version information")
        else:
            sys.stdout.write("%s\n" % (bytestostr(v),))
            self.quit(0)
...
class ControlXpraClient(CommandConnectClient):
    ...
    def timeout(self, *_args):
        self.warn_and_quit(1, "timeout: server did not respond")

    def do_command(self):
        cr = self.server_capabilities.listget("command_response")
        if cr is None:
            self.warn_and_quit(1, "server does not support control command")
            return
        code, text = cr
        text = bytestostr(text)
        if code!=0:
            log.warn("server returned error code %s", code)
            self.warn_and_quit(1, " %s" % text)
            return
        self.warn_and_quit(0, text)
...
class PrintClient(SendCommandConnectClient):
    ...
    def setup_connection(self, conn):
        log.error("gobject setup connection")
    ...
    def do_command(self):
        ...
        if not printing:
            self.warn_and_quit(1, "server does not support printing")
            return
